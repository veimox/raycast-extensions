...
import { DEFAULT_IMAGE_MODEL, useModel } from "./hooks/useModel";
...
export default function Ask(props: { conversation?: Conversation }) {
  ...
  const chats = useImageChat<Chat>(props.conversation ? props.conversation.chats : []);
  ...
  useEffect(() => {
    if (models.data && conversation.chats.length === 0) {
      const defaultUserModel = models.data.find((x) => x.id === DEFAULT_IMAGE_MODEL.id) ?? conversation.model;
  ...
  return (
    <List
      ...
      searchBarPlaceholder={chats.data.length > 0 ? "Generate another image..." : "Generate a image..."}
    >
      <ChatView
        data={chats.data}
